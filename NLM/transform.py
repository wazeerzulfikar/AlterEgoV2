...
def default_transform_new(data, sample_rate=250, **kwargs):
    ...
    notch1 = 60 #59.9
    num_times = 3 #2
    freqs = list(map(int, list(map(round, np.arange(1, sample_rate/(2. * notch1)) * notch1))))
    ...
    def ricker_wavelet(n, sigma):
        return np.array(list(map(lambda x: ricker_function(x, sigma), range(-n//2, n//2+1))))
...
def augment(data, arg_sets, include_original=False):
    ...
    augmented = np.array(list(map(lambda x: x[0](data, *x[1], modify=False, **x[2]), arg_sets)))
    sequence_groups = []
...
def apply_recursively(data, fn, modify):
    ...
    if modify:
        ...
        return data
    # return np.array(list(map(lambda x: apply_recursively(x, fn, modify), data))
    return [apply_recursively(i, fn, modify) for i in data]
